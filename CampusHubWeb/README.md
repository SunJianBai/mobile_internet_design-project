...
`CampusHubWeb` 是“CampusHub”系统的前端应用，面向在校同学提供一站式的校园社交与约伴服务，主要功能包括：

- 账号登录 / 注册 / 忘记密码找回
- 首页聚合：快捷发布动态/订单、我的订单概览、动态流
- 订单模块：订单筛选、列表、详情、申请 / 审核 / 撤销 / 取消、部分字段编辑；详情页展示发布者邮箱，且对已通过的申请人展示邮箱，便于线下联系
- 动态模块：动态列表、详情、发布包含图片的动态、点赞与评论（含楼中楼）、删除
- AI 问询：与后端 AI 服务对话，并基于自然语言对话生成“订单预填草稿”并一键跳转到创建页
- 个人中心：查看并编辑个人基础信息、头像等

整个应用与后端 `CampusHubBackend` 通过 REST API 对接，前端只负责 UI 呈现和交互逻辑。
...
- Node.js：建议 `>= 18.x`（当前使用 18+ 与 Vite 7.2.4 测试通过）
...
- **前端启动 404 / 白屏**：
  ...
  - 确认已在 `CampusHubWeb` 下执行 `npm install`；
  - 确认 Node.js 版本 >= 18；
  - 查看终端是否有 Vite 报错；
...
- 订单详情（/orders/:id）
  ...
  - 功能：
    - 展示发布者信息（头像、昵称、ID、邮箱等）、订单基础信息（活动、时间、地点、人数、备注、状态、创建/更新时间）；
    - 发布者或管理员可在详情页编辑部分字段：地点、开始时间、人数上限、备注，并有本地“修改记录”列表；
    - 右侧“申请信息”卡片展示所有申请：申请人头像、昵称、申请时间、状态；
    - 对已通过的申请人，显示其邮箱（基于用户 ID 额外拉取用户详情）；
...
- AI 问询页（/ai）
  ...
  - 页面：`/ai`（[src/views/AIView.vue](src/views/AIView.vue)），路由配置见 [src/router/index.js](src/router/index.js)，需登录后访问（`meta.requiresAuth = true`）。
  - 对话与渲染：
    - 使用后端 `/ai/chat/full` 非流式接口（`services/ai.fullChat`），一次性获取完整回复；
    - 会话以消息列表形式展示，区分 `user/assistant` 角色；
    - 内置轻量 Markdown 渲染（标题、粗体、斜体、链接、行内代码、代码块、列表等）、HTML 转义与代码块高亮样式；
    - 输入框支持多行文本，`Enter` 发送、`Shift+Enter` 换行，并显示字数统计（上限 4000 字符）。
  - 体验细节：
    - 发送后会在列表中插入一条“AI 正在生成回复”的占位消息，显示三点加载动画，待接口返回后替换为正式文本；
    - 支持本地草稿：输入自动写入 `localStorage.ai_draft`，刷新或误操作后可恢复未发送内容；
    - 所有错误（网络异常、后端错误等）会在当前助手气泡中以“错误：xxx”形式展示。 

- AI 驱动的“订单预填草稿”

  - 技术位置：
    - 本地解析服务：`src/mcp/orderAssistant.js`，提供 `orderAssistantService`；
    - 本地 MCP 注册中心：`src/utils/mcp.js`，当前主要由 `AIView` 直接调用服务函数，后续可扩展为统一注册/调度中心；
    - 入口组件：`src/views/AIView.vue`，在每次普通 AI 回复完成后，根据本轮用户输入决定是否追加“订单草稿建议”。
  - 触发条件：
    - 当本轮用户输入中包含“帮我约 / 帮我生成订单 / 帮我下一个订单 / 帮我预填订单 / 帮我预填写订单 / 生成一个订单 / 创建一个订单”等中文短语之一时，AI 回复完成后会自动尝试生成订单草稿；
    - 触发逻辑位于 `AIView` 的 `shouldTriggerOrderMcpFromText(text)` 中，只基于用户原始输入文本判断，与 AI 回复内容无关。
  - 解析与字段映射：
    - `orderAssistantService` 会优先使用当前输入框文本（`inputText`），否则回退到对话历史中最后一条 `user` 消息的内容；
    - 通过中文关键词和正则从自然语言中抽取：
      - 活动类型：如“篮球 / 羽毛球 / 吃饭 / 自习 / 看电影 / 跑步 / 游戏 / 开黑”等，映射到后端 `ActivityType` 枚举；
      - 校区：如“良乡 / 中关村 / 珠海 / 西山”等，映射到后端 `Campus` 枚举；
      - 性别要求：如“只要女生 / 仅限女生 / 女生优先 / 只要男生 / 男生优先”等，映射到 `GenderRequire`（`FEMALE/MALE/ANY`）；
      - 人数上限：匹配“3人 / 4 位 / 5 个”等表达，限制在 1～20 之间；
      - 地点：从“在XXX”中截取 2～20 个中英文或数字字符作为地点描述；
      - 开始时间：支持“今天/明天/后天 + HH点/HH:MM”形式，将结果格式化为 `yyyy-MM-dd HH:mm:00`，交给创建订单页直接使用；
      - 备注：兜底将整句自然语言作为备注传入。
    - 解析结果组装为 `draft`（结构与创建订单页期望一致）和 `query`（用于 `/orders/create` 的 URL 查询参数）。
  - 对话中的表现形式：
    - 在 AI 文本回复后，`AIView` 会额外插入一条 `type: 'order-draft-suggestion'` 的助手消息：
      - 以列表形式预览解析出的“活动类型 / 校区 / 地点 / 时间 / 性别要求 / 人数上限”；
      - 提供“点击跳转并预填写订单”按钮；
    - 用户点击按钮后，通过 `router.push({ path: '/orders/create', query })` 跳转到创建订单页，创建订单页从 query 中读取预填表单字段。

> 当前订单草稿生成完全基于前端规则实现，不依赖 AI 的结构化输出；对复杂的自然语言时间（如“下周三下午”、“本周末”等）的支持有限，建议在描述中尽量包含明确的日期与时间点。
...
- 在 `beforeEach` 中：
  - 根据 `to.meta.title` 设置页面标题 `${title} - CampusHub`；
