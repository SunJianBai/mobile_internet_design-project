...
MAP_UI_RESPONSE_RULES = """

## 地图交互卡片输出协议（非常重要）
- 当本轮意图是地点查询、店铺推荐、路线或附近搜索时，最终回复必须尽量包含可交互地图块。
- 如果工具结果里有经纬度，至少为最推荐的 1 个地点输出地图块，最多输出 3 个。
- 地图块必须单独占一行，语法严格为：`:::map{lng=经度 lat=纬度 zoom=15 title=地点名称}`。
- 推荐店铺时，不要只给外部跳转链接；先给文字推荐，再紧跟对应地图块。
- 如果工具没有返回经纬度，要明确说明“当前结果缺少经纬度，无法直接渲染地图”。
"""
...
def build_main_agent_prompt(user_info: dict, memories: list) -> str:
    ...
    return f"""你是「CampusHub」智能助手，服务于北京理工大学学生。
...
## 返回格式补充（给主Agent使用）
- 对每个 POI 结果尽量保留：地点名称、地址、经度、纬度。
- 如果工具返回 location 字段，必须原样写出，例如：经纬度：116.123456,39.123456。
- 不要只返回店名和地址，否则前端无法渲染地图卡。"""


# ==================== 记忆提取 ====================

MEMORY_EXTRACTION_PROMPT = """根据以下用户与AI的对话，提取关于用户的长期稳定信息（如偏好、习惯、个人资料等）。

用户消息: {user_message}
AI回复: {assistant_reply}

只保留以后多轮对话仍有价值的信息。不要提取以下内容：
- 本轮一次性查询、搜索、推荐、地图结果、路线、坐标、天气结果
- 待确认草稿、缺失字段、临时计划、今天/今晚/明天这类短期安排
- AI 推荐出的店铺、地点、订单、动态内容，除非用户明确说“以后记住/我偏好/我经常”
- 只是复述用户当前想做什么的内容，例如“用户正在寻找按摩店”
- 不要猜测用户状态；包含“可能/似乎/大概/不确定/消息不明确/测试系统反应”的内容应视为无效
- 不要把 “none”“没有提取到关于用户的事实性信息” 这类占位语包装成记忆

每轮最多返回 2 条。请以 JSON 数组格式返回提取的信息，每条包含 category 和 content 字段。
